...
// ── Reports-To lookup (search KYC contacts, auto-fill manager fields) ─────────
function ReportsToLookupField({
  ...
  // Sync if form values change externally (e.g. EditPage initialValues)
  useEffect(() => {
    setInputText(values.reportingToText ?? "");
    setSelectedId(values.reportingTo ?? "");
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const { data: contactsData, isLoading } = useContacts({
    search: inputText.trim().length >= 1 ? inputText.trim() : undefined,
    limit: 50,
  });
  ...
  const fillFromContact = (c: any) => {
    ...
    setIsOpen(false);
    setFieldValue("reportingTo",      c.id);
    setFieldValue("reportingToText",  fullName);
    setFieldValue("reportingFunction", c.jobFunction  ?? "");
    setFieldValue("managerDesignation", c.designation ?? "");
    setFieldValue("managerName",       fullName);
    setFieldValue("managerEmail",      c.email        ?? "");
    setFieldValue("managerMobile",     c.mobileNumber1 ?? "");
    setFieldValue("managerLocation",   "");
    setFieldValue("managerFunction",   c.jobFunction  ?? "");
  };
  ...
  const clearSelection = () => {
    ...
    setFieldValue("reportingToText",   "");
    setFieldValue("reportingFunction", "");
    setFieldValue("managerDesignation","");
    setFieldValue("managerName",       "");
    setFieldValue("managerEmail",      "");
    setFieldValue("managerMobile",     "");
    setFieldValue("managerLocation",   "");
    setFieldValue("managerFunction",   "");
  };

  const handleInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    const v = e.target.value;
    setInputText(v);
    // If the user edits the text after picking a contact, detach the link
    if (selectedId) {
      setSelectedId("");
      setFieldValue("reportingTo", "");
    }
    setFieldValue("reportingToText", v);
    setIsOpen(true);
  };

  const isAutoFilled = !!selectedId;
    ...
    <div className="relative col-span-2" ref={dropdownRef}>
      ...
      <div className="relative">
        <input
          ...
          className={`${inputClass(!!errors.reportingToText && !!touched.reportingToText)} pr-16`}
        />
        <div className="absolute right-2.5 top-1/2 -translate-y-1/2 flex items-center gap-1.5">
          ...
          {inputText && (
            <button type="button" onClick={clearSelection} className="p-0.5 hover:bg-slate-100 rounded-full transition-colors" title="Clear">
              <svg className="w-3.5 h-3.5 text-slate-400 hover:text-slate-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
      ...
      {isOpen && inputText.trim().length >= 1 && (
        <div className="absolute z-50 w-full mt-1.5 bg-white border border-slate-200 rounded-xl shadow-lg max-h-60 overflow-y-auto p-2 flex flex-col gap-1">
          {isLoading ? (
            <div className="text-xs text-slate-400 py-3 text-center">Searching contacts…</div>
          ) : contacts.length === 0 ? (
            <div className="flex flex-col items-center gap-1.5 py-3">
              <p className="text-xs text-slate-400">No contacts found for "{inputText}"</p>
              <p className="text-[10px] text-slate-400">The name will be saved as free text — a linked contact can be added later.</p>
            </div>
          ) : (
            contacts.map((c) => {
              const fullName = [c.firstName, c.lastName].filter(Boolean).join(" ");
              return (
                ...
                >
                  ...
                  <div className="flex flex-col min-w-0">
                    ...
                    <span className="text-[10px] text-slate-400 truncate">{c.designation} · {c.email}</span>
                  </div>
                </div>
              );
            })
...
// ── Single-contact lookup (Account Manager / CSM / etc.) ─────────────────────
function SingleContactLookupField({
  name, label, errors, touched, setFieldValue, values
}: {
  name: string; label: string; errors: any; touched: any;
  setFieldValue: (field: string, value: any) => void;
  values: any;
    ...
    <div className="relative" ref={dropdownRef}>
      ...
      >
        {selectedId && selectedContact ? (
          <div className="flex items-center justify-between w-full">
            <div className="flex items-center gap-2 min-w-0">
              <div className="w-5 h-5 rounded-full bg-indigo-50 border border-indigo-100 flex items-center justify-center text-[10px] font-bold text-indigo-600 shrink-0">
                {(selectedContact.firstName || "C").charAt(0).toUpperCase()}
              </div>
              <span className="text-slate-700 text-sm font-medium truncate">
                ...
                <span className="text-slate-400 font-normal ml-1 text-xs">· {selectedContact.designation}</span>
              </span>
            </div>
            <button type="button" onClick={(e) => { e.stopPropagation(); setFieldValue(name, ""); setSearch(""); }}
              className="p-1 hover:bg-slate-100 rounded-full transition-colors shrink-0" title="Clear">
              <svg className="w-3.5 h-3.5 text-slate-400 hover:text-slate-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
          ...
          <div className="flex items-center justify-between w-full">
            <span className="text-slate-400 text-sm">Search contacts…</span>
            <Search size={14} className="text-slate-400" />
          </div>
        )}
      </div>
      ...
      {isOpen && (
        <div className="absolute z-50 w-full mt-1.5 bg-white border border-slate-200 rounded-xl shadow-lg max-h-60 overflow-y-auto p-2 flex flex-col gap-1">
          ...
          {isLoading ? (
            <div className="text-xs text-slate-400 py-3 text-center">Searching…</div>
          ) : contacts.length === 0 ? (
            <div className="text-xs text-slate-400 py-3 text-center">No contacts found</div>
          ) : (
            contacts.map((c) => {
              const fullName = [c.firstName, c.lastName].filter(Boolean).join(" ");
              return (
                <div key={c.id} onClick={() => { setFieldValue(name, c.id); setIsOpen(false); setSearch(""); }}
                  className={`flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-xs cursor-pointer transition-colors ${
                    selectedId === c.id ? "bg-indigo-50 text-indigo-700 font-semibold" : "text-slate-600 hover:bg-slate-50"
                  }`}>
                  <div className="w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center text-[10px] font-semibold text-slate-600 shrink-0">
                    {(c.firstName || "C").charAt(0).toUpperCase()}
                  </div>
...
// ── Multi-contact lookup (Account Team) ───────────────────────────────────────
function MultiContactLookupField({
  name, label, errors, touched, setFieldValue, values
}: {
  name: string; label: string; errors: any; touched: any;
  setFieldValue: (field: string, value: any) => void;
  ...
  // keep a map of id → name for displaying chips even when not in search results
  const [nameMap, setNameMap] = useState<Record<string, string>>({});
  useEffect(() => {
    const next: Record<string, string> = { ...nameMap };
    contacts.forEach((c) => {
      next[c.id] = [c.firstName, c.lastName].filter(Boolean).join(" ");
    });
    setNameMap(next);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [contacts]);
      ...
      <div onClick={() => setIsOpen(!isOpen)}
        ...
        <div className="flex flex-wrap gap-1 items-center">
          {selectedIds.length === 0 ? (
            <span className="text-slate-400 text-sm">Search and add contacts…</span>
          ) : (
            selectedIds.map((id) => (
              <span key={id} className="inline-flex items-center gap-1 bg-indigo-50 text-indigo-700 text-[11px] font-semibold px-2 py-0.5 rounded-md border border-indigo-100">
                {nameMap[id] ?? id.slice(0, 8) + "…"}
                <button type="button" onClick={(e) => { e.stopPropagation(); toggle(id); }} className="hover:text-indigo-900">&times;</button>
              </span>
            ))
          )}
        </div>
            ...
            contacts.map((c) => {
              ...
              return (
                <div key={c.id} onClick={() => toggle(c.id)}
                  className={`flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-xs cursor-pointer transition-colors ${
                    isSelected ? "bg-indigo-50/50 text-indigo-700 font-semibold" : "text-slate-600 hover:bg-slate-50"
                  }`}>
                  <input type="checkbox" checked={isSelected} readOnly
                    className="w-3.5 h-3.5 rounded border-slate-300 text-indigo-600 accent-indigo-500 cursor-pointer shrink-0" />
                  <div className="w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center text-[10px] font-semibold text-slate-600 shrink-0">
                    {(c.firstName || "C").charAt(0).toUpperCase()}
                  </div>
                  <div className="flex flex-col min-w-0">
...
const INITIAL: Record<string, unknown> = {
  accountId: "", contactType: "", kycCategory: "", firstName: "", lastName: "",
  email: "", mobileNumber1: "", mobileNumber2: "", landlineNumber: "",
  jobFunction: "", designation: "", department: "", employeeLevel: "", employeeId: "",
  joiningDate: "", decisionAuthority: "", preferredContactTime: "", timeZone: "",
  languagePreference: "", linkedinProfile: "", assistantName: "", assistantContact: "",
  notes: "", reportingTo: "", reportingFunction: "", reportingToText: "",
  managerLocation: "", managerFunction: "", managerDesignation: "", managerName: "",
  managerEmail: "", managerMobile: "", engagementFrequency: "",
  lastMeetingDate: "", nextFollowUpDate: "", contactPriority: "",
  lastInteractionDate: "", lastInteractionNotes: "", influenceLevel: "",
  decisionMakingPower: "", customerSentiment: "", associationStatus: "",
  influencerScore: "", relationshipStrength: "", relationshipScore: "", dataQualityScore: "",
  buyingRole: [],
  preferredCommMode: [],
  budgetOwnership: false, technicalEvaluator: false, marketingConsent: false,
  doNotContact: false,
  dob: "",
  anniversaryDate: "",
  personalInterests: [],
  accountManager: "", customerSuccessManager: "", preSalesOwner: "", relationshipOwner: "",
  accountTeam: [],
};
    ...
    <div className="flex flex-col gap-6 max-w-4xl pb-10">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800 tracking-tight">New KYC</h1>
            <p className="text-xs text-slate-400">Fill in the details to create a new KYC record</p>
          </div>
        </div>
      </div>
        ...
        {({ errors, touched, isSubmitting, setFieldValue, values }) => {
          ...
          return (
            <Form className="flex flex-col gap-5">
              ...
              {/* 1 · Core Identity */}
              <Section title="1 — Contact Identity">
                <FullRow>
                  <AccountLookupField
                    name="accountId"
                    label="Account"
                ...
                <FF name="contactType" label="Contact Type" req as="select" errors={E} touched={T}>
                  <option value="">— Select —</option>
                  {CONTACT_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
                </FF>
                <FF name="kycCategory" label="KYC Category" req as="select" errors={E} touched={T}>
                  <option value="">— Select —</option>
                  {KYC_CATEGORIES.map((t) => <option key={t} value={t}>{t}</option>)}
                </FF>
                <FF name="firstName"   label="First Name"  req placeholder="John"  errors={E} touched={T} />
                <FF name="lastName"    label="Last Name"       placeholder="Smith" errors={E} touched={T} />
                <FF name="email"       label="Email Address" req type="email" placeholder="john@company.com" errors={E} touched={T} />
                <FF name="mobileNumber1" label="Mobile Number 1" req placeholder="+91 98000 00000" errors={E} touched={T} />
                <FF name="mobileNumber2" label="Mobile Number 2"    placeholder="+91 98000 00001" errors={E} touched={T} />
                <FF name="landlineNumber" label="Landline"          placeholder="+91 22 4000 0000" errors={E} touched={T} />
              </Section>

              {/* 2 · Professional Profile */}
              <Section title="2 — Professional Profile">
                <FF name="jobFunction" label="Job Function" req as="select" errors={E} touched={T}>
                  <option value="">— Select —</option>
                  {JOB_FUNCTIONS.map((j) => <option key={j} value={j}>{j}</option>)}
                </FF>
                <FF name="designation" label="Designation" req placeholder="VP Sales" errors={E} touched={T} />
                ...
                <FF name="employeeId"  label="Employee ID" placeholder="EMP-001" errors={E} touched={T} />
                <FF name="joiningDate" label="Joining Date" type="date" errors={E} touched={T} />
                <FF name="linkedinProfile" label="LinkedIn Profile URL" type="url" placeholder="https://linkedin.com/in/..." errors={E} touched={T} />
              </Section>
              ...
              <Section title="3 — Buying Influence & Role">
                ...
                <MultiSelectField
                  ...
                  values={values}
                />
                <FF name="influencerScore"    label="Influencer Score (0-100)"     type="number" placeholder="0" errors={E} touched={T} />
                <FF name="relationshipStrength" label="Relationship Strength (0-100)" type="number" placeholder="0" errors={E} touched={T} />
              ...
              <Section title="4 — Communication Preferences">
                <MultiSelectField
                  ...
                  values={values}
                />
                <FF name="preferredContactTime" label="Preferred Contact Time" placeholder="e.g. 10 AM – 12 PM IST" errors={E} touched={T} />
              ...
              <Section title="5 — Personal Details">
                ...
                <FF name="assistantName"    label="Assistant Name"    placeholder="Jane Doe" errors={E} touched={T} />
                <FF name="assistantContact" label="Assistant Contact" placeholder="+91 98000 00002" errors={E} touched={T} />
                <MultiSelectField
                ...
                />
                <FullRow>
                  <FF name="notes" label="Notes" as="textarea" rows={3} placeholder="Additional notes about this contact…" errors={E} touched={T} />
                </FullRow>
              </Section>

              {/* 6 · Reporting Hierarchy */}
              <Section title="6 — Reporting Hierarchy">
                <ReportsToLookupField
                  errors={E} touched={T}
                  setFieldValue={setFieldValue}
                  values={values}
                />
                {(values.reportingTo as string) && (
                  <div className="col-span-2 flex items-center gap-2 px-3 py-2 bg-emerald-50 border border-emerald-200 rounded-xl text-xs text-emerald-700">
                    ...
                    Manager fields below have been auto-populated from the selected contact. You may edit them if needed.
                  </div>
                )}
              ...
              <Section title="7 — Account Ownership & Engagement">
                ...
                <FullRow>
                  <FF name="lastInteractionNotes" label="Last Interaction Notes" as="textarea" rows={2} placeholder="Summary of last interaction…" errors={E} touched={T} />
                </FullRow>
              </Section>

              {/* 8 · Scoring & Analytics */}
              <Section title="8 — Scoring & Analytics">
                <FF name="relationshipScore"  label="Relationship Score (0-100)"  type="number" placeholder="0" errors={E} touched={T} />
                <FF name="dataQualityScore"   label="Data Quality Score (0-100)"  type="number" placeholder="0" errors={E} touched={T} />
                ...
                <div className="col-span-2 flex items-start gap-2.5 px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-500">
                  ...
                  <span><strong>Duplicate Contact Check</strong> and <strong>Account Association Status</strong> are automatically identified by the system and displayed in the KYC detail view.</span>
                </div>
              </Section>
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                ...
                >
                  ...
                  {isSubmitting ? "Saving…" : "Create KYC"}
                </motion.button>
              </div>
            </Form>
          );
